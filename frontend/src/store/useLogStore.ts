...
export const useLogStore = create<LogStore>((set, get) => ({
  ...
  removeSearchMessage: (message) => {
    set((state) => ({
      filters: {
        ...state.filters,
        searchMessages: state.filters.searchMessages.filter(
          (m) => m !== message
        ),
      },
    }));
  ...
  executeCloudWatchQuery: async (parsedQuery: ParsedQuery) => {
    ...
    try {
      const filterCmds = parsedQuery.commands.filter(
        (cmd: any) => cmd.type === "filter"
      );
      const sortCmd = parsedQuery.commands.find(
        (cmd: any) => cmd.type === "sort"
      );
      const limitCmd = parsedQuery.commands.find(
        (cmd: any) => cmd.type === "limit"
      );
      ...
      // Handle filters
      const serviceFilter = filterCmds.find(
        (cmd: any) =>
      ...
      const levelFilters = filterCmds.filter(
        (cmd: any) =>
          cmd.params.field === "level" && cmd.params.operator === "="
      );
      if (levelFilters.length > 0) {
        requestBody.levels = levelFilters.map((cmd: any) => cmd.params.value);
      }

      // Handle message search
      const messageFilters = filterCmds.filter(
        (cmd: any) =>
          (cmd.params.field === "message" || cmd.params.field === "@message") &&
          (cmd.params.operator === "=" || cmd.params.operator === "like")
      );
      if (messageFilters.length > 0) {
        requestBody.message_contains = messageFilters.map(
          (cmd: any) => cmd.params.value
        );
      }

      // Handle metadata filters
      const metadataFilters = filterCmds.filter(
        (cmd: any) =>
      ...
      if (metadataFilters.length > 0) {
        ...
        metadataFilters.forEach((cmd: any) => {
      ...
      set({
        ...
        filters: {
          ...
          page: 1,
          limit: limitCmd?.params.limit || 20,
          sortOrder: (sortCmd?.params.direction === "desc" ? "desc" : "asc") as
            | "asc"
            | "desc",
        },
      });
