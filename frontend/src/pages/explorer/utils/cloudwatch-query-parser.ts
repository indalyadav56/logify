 ...
 */
export function convertToLogifyFilters(parsedQuery: ParsedQuery): Record<string, any> {
...
function handleFilterCommand(command: QueryCommand, filters: Record<string, any>): void {
  ...
  else if (!field.startsWith('@') && field !== 'level' && field !== 'service' && field !== 'message') {
    filters.metadata[field] = value;
  }
}

function handleSortCommand(command: QueryCommand, filters: Record<string, any>): void {
...
function handleLimitCommand(command: QueryCommand, filters: Record<string, any>): void {
